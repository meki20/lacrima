...
export default function SettingsForm({ initial }: { initial: Settings }) {
  ...
  const save = (patch: Partial<Settings>) => {
    ...
    void fetch("/api/settings", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(patch) });
  };
  return <div className="settings-form">
    <section className="settings-card"><div><h2>Playback</h2><p>Defaults for every new episode.</p></div>
      ...
      <label>Caption size<select value={settings.caption_scale} onChange={(e) => save({ caption_scale: Number(e.target.value) })}><option value="1">Normal</option><option value="1.2">Large</option><option value="1.45">Extra large</option></select></label>
    </section>
